...
export async function findUser(options?: FindConditions<User>): Promise<User | void> {
  const user: User | undefined = await userRepository().findOne(options);
  if (user === undefined) {
...
export async function modify(user: User): Promise<User | void> {
